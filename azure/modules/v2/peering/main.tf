    ...
    for k1, v1 in var.peerings : [
      for v2 in v1.src : [
        for v3 in v1.dst : {
          ...
          src                   = v2
          dst                   = v3
          use_remote_gateway    = v1.use_remote_gateway
        }
      ]
    ]
  ]))
...
resource "azurerm_virtual_network_peering" "peering-dst" {
  ...
  allow_gateway_transit     = each.value.use_remote_gateway == true ? true : false
}
...
resource "azurerm_virtual_network_peering" "peering-src" {
  ...
  use_remote_gateways       = each.value.use_remote_gateway == true ? true : false
}